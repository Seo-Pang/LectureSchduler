...
Schedule DynamicProgrammingScheduling(LecArray lecture[MAX_LECTURE])
{
	...
	{
		for (int DLC = 1; DLC < MAX_CREDIT+1; DLC++) //임시 가방 무게에서 최대 가방 무게까지 for문에 의해 설정된 변수, Dynamic Limit Credit
		{
			...
			if (DLC < lecture[i].credit) //학점을 넣을 수 없을 떄
